      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">
            Inspeção, identificação e liberação — use o scanner ou avance passo a passo na tabela.
          </p>
        </div>
      ...
      <Card>
        ...
        {loading ? (
          ...
          <p className="text-sm text-slate-500">
            Nenhuma unidade pendente. Após receber uma PO em{" "}
            <Link href="/compras" className="text-blue-600 hover:underline">
              Compras
            </Link>
            , as unidades aparecem aqui.
          </p>
        ) : (
          <Table
            headers={["", "Código", "SKU", "PO", "Status", "Próximo", "Custo", "Recebido", ""]}
            rows={items.map((item) => [
              <input
                key={`cb-${item.id}`}
                type="checkbox"
